"""
generate_bpsp_seed.py
Gera um arquivo SQL comprimido (gzip) com os dados do BPSP para carregamento
no banco PostgreSQL do equipamento servidor da simulação federada.

Lê os CSVs diretamente do ZIP (sem extração em disco) e grava blocos
COPY FROM stdin para as 4 tabelas do schema MOSAIC-FL:

  clinical.patients          (~39.000 linhas)
  clinical.attendances       (~217.991 linhas)
  metrics.clinical_outcomes  (~217.991 linhas)
  metrics.exam_records       (~6.329.103 linhas)

O arquivo gerado (scripts/db/seeds/bpsp_seed.sql.gz) é carregado com:
    make server-load-bpsp

Por que BPSP e não Einstein?
  O Einstein possui 3,4M de exames (mais que o BPSP), mas não tem arquivo de
  desfechos. O SequencePipeline exige desfechos para gerar os labels de
  prognóstico (alta / internação prolongada / UTI / óbito). O BPSP é o hospital
...
# ---------------------------------------------------------------------------
# Helpers (idênticos ao generate_hsl_seed.py)
# ---------------------------------------------------------------------------
...
def generate_exams(
    ...
    # a guarda de lá só protege integration/fapesp/loader.py, não este pipeline,
    # que é o que os targets client-generate-seed/server-generate-seed realmente
    # usam). Sem uma etapa de revisão prévia, o melhor que dá pra fazer aqui é
    # avisar alto — revise este log ANTES de rodar server-load-bpsp.
    ...
    try:
        ...
        for chunk in pd.read_csv(
            ...
            if mapping is None:
                mapping = resolver_exams.resolve(list(chunk.columns))
            ...
            def gcol(c):
                ...
                return chunk[c_] if c_ and c_ in chunk.columns else \
                    pd.Series([None] * len(chunk), index=chunk.index)
            ...
            ref_s    = gcol("reference_range")[valid].fillna("").astype(str).apply(parse_reference_range)
            ...
            if new_suspicious:
                ...
                logger.warning(
                    ...
                    "knowledge.term_dictionary ANTES de rodar server-load-bpsp, ou use o "
            ...
            for pid, d, rt, v, rl, rh, ori, u, eg, ai, can, ph in zip(
                ...
                ai_val = str(ai).strip() if pd.notna(ai) and str(ai) not in ("nan", "None") else None
                if ai_val and ai_val not in att_ids:
                    ai_val = None

                eg_val = str(eg).strip() if pd.notna(eg) and str(eg) not in ("nan", "None") else None
    ...
    if suspicious_seen:
        logger.warning(
            ...
            "Revise antes de server-load-bpsp (ver aviso acima pra cada um).",
            len(suspicious_seen), sorted(suspicious_seen),
        )


resolver_exams = ColumnResolver(
    CLINICAL_SEMANTIC_MAP,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Gera SQL seed do BPSP para o banco servidor da simulação FL."
    )
    parser.add_argument(
        "--data-dir", required=True,
        help="Diretório com os ZIPs FAPESP (deve conter BPSP.zip).",
    )
    parser.add_argument(
        "--output",
        default=str(Path(__file__).parent / "seeds" / "bpsp_seed.sql.gz"),
        help="Arquivo de saída (.sql.gz). Padrão: scripts/db/seeds/bpsp_seed.sql.gz",
    )
    parser.add_argument(
        "--db-url",
    ...
    with gzip.open(output, "wb") as gz:
        gz.write(b"-- MOSAIC-FL BPSP Seed\n")
        gz.write(b"-- Hospital: Beneficencia Portuguesa SP (BPSP) | Simulacao servidor FL\n")
        gz.write(b"-- Gerado por scripts/db/generate_bpsp_seed.py\n")
        ...
        # Registra este banco como servidor BPSP (sobrepõe o default 'client/HSL' da migration 010)
        gz.write(b"INSERT INTO clinical.simulation_node_config\n")
        gz.write(b"    (id, node_role, hospital_id, data_source, description)\n")
        gz.write(b"VALUES (\n")
        gz.write(b"    'current', 'server', 'BPSP', 'FAPESP',\n")
        gz.write(b"    'Simulacao federada TCC - Desktop i9-13900K / 32 GB. "\
                 b"Dados: Beneficencia Portuguesa SP - BPSP.zip. "\
                 b"Cliente: Notebook Dell Inspiron 5402 (i7-1165G7 / 16 GB) rodando HSL.'\n")
        gz.write(b")\n")
        gz.write(b"ON CONFLICT (id) DO UPDATE SET\n")
        gz.write(b"    node_role    = EXCLUDED.node_role,\n")
        gz.write(b"    hospital_id  = EXCLUDED.hospital_id,\n")
        gz.write(b"    data_source  = EXCLUDED.data_source,\n")
        gz.write(b"    description  = EXCLUDED.description,\n")
        gz.write(b"    registered_at = NOW();\n\n")
        ...
        logger.info("3/3 Gerando metrics.exam_records (~6,3 M linhas — pode levar vários minutos)...")
