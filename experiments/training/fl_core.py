"""
Núcleo do aprendizado federado: agregação FedAvg, avaliação global e loops de treinamento.

  aggregate_fedavg              — média ponderada de state_dicts
...
from mosaicfl.core.config import (
    ...
    DEVICE, FL_DB_URL, LOCAL_EPOCHS, MODEL_CFG, NUM_ROUNDS, USE_RAY,
)
...
def run_federated_learning_manual(
    ...
    logger.info(
        f"Checkpoint salvo no store ({type(checkpoint_store).__name__}, "
        f"vocab_size={len(vocab or {})}, T={scaler.T:.4f})"
    )
