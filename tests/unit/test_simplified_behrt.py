...
class TestSimplifiedBEHRT:
    ...
    def test_forward_returns_correct_shape(self, model):
    ...
    def test_cls_token_prepended(self, model):
        x = torch.randint(1, VOCAB_SIZE, (2, 16))
    ...
    def test_masked_mean_pool_all_padding_no_nan(self, model):
    ...
    def test_cls_token_is_parameter(self, model):
        assert isinstance(model.cls_token, torch.nn.Parameter)

    def test_pre_classifier_applied(self, model):
        x = torch.randn(2, EMBED_DIM)
        out = model.pre_classifier(x)
